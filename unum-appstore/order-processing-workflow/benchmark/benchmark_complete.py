...
def configure_mode(mode: str, function_arns: Dict[str, str]):
    ...
    SlowChainEnd) - it determines whether _run_fan_in_eager or
...
def run_single_iteration(run_id: int, mode: str, order_id: str, function_arns: Dict[str, str],
    ...
    # Compare Aggregator invocation time vs SlowChainEnd completion time:
    #   - If Aggregator was invoked BEFORE SlowChainEnd finished → FastProcessor (eager)
    #   - If Aggregator was invoked AFTER SlowChainEnd finished → SlowChainEnd (classic)
    agg_m = metrics.get('Aggregator', LambdaMetrics(''))
    slow_end_m = metrics.get('SlowChainEnd', LambdaMetrics(''))
    if agg_m.invocation_time_ms > 0 and slow_end_m.invocation_time_ms > 0:
        slow_end_completed_ms = slow_end_m.invocation_time_ms + int(slow_end_m.duration_ms)
        if agg_m.invocation_time_ms < slow_end_completed_ms:
            run.invoker_branch = 'FastProcessor'
        else:
            run.invoker_branch = 'SlowChainEnd'
    elif agg_m.invocation_time_ms > 0:
        # No SlowChainEnd metrics - infer from aggregator delay
        run.invoker_branch = 'FastProcessor' if run.aggregator_invocation_delay_ms < 5000 else 'SlowChainEnd'
...
def print_summary(summary: BenchmarkSummary):
    ...
    if summary.successful_runs > 0:
        ...
        print(f"  Slow Chain Total: {summary.avg_slow_chain_total_ms:8.2f}ms")
        print()
...
def compare_results(classic: BenchmarkSummary, future: BenchmarkSummary):
    ...
    print(f"  CLASSIC:       {classic.aggregator_invocation_mean_ms:8.2f}ms  (invoked by SlowChainEnd)")
    ...
    print(f"  hiding cold start latency behind slow chain execution.")
    print(f"{'='*70}\n")
...
def generate_charts(classic: BenchmarkSummary, future: BenchmarkSummary,
    ...
    for i, (bar, val) in enumerate(zip(bars, agg_means)):
        invoker = "SlowChainEnd" if i == 0 else "FastProcessor"
    ...
    classic_times = [
        ...
        classic.avg_slow_chain_start_ms, classic.avg_slow_chain_mid_ms,
        classic.avg_slow_chain_end_ms, classic.avg_aggregator_ms
    ]
    future_times = [
        future.avg_trigger_ms, future.avg_fast_processor_ms,
        future.avg_slow_chain_start_ms, future.avg_slow_chain_mid_ms,
        future.avg_slow_chain_end_ms, future.avg_aggregator_ms
    ]
