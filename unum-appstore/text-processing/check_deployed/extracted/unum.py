...
class UnumContinuation(object):
    ...
    def _run_fan_in(self, user_function_output, session, next_payload_metadata, input_payload, unum_index_list, **kwargs):
        ...
        NOTE: All branches participate in fan-in synchronization by marking
        themselves as ready. The conditional only determines which branch
        will invoke the aggregation function once all branches are ready.
        '''
        ...
        # because all branches fan into the SAME aggregation function instance, not
        # separate instances per branch. The Fan-out index is only relevant for
        # identifying which branch we are, not for naming the aggregation function.
        payload = {}
        for f in next_payload_metadata:
            if next_payload_metadata[f] != None:
        ...
        if self.debug:
            print(f'[DEBUG] {self.my_node_name}-{unum_index_list} fan-in aggregation_function_instance_name: {aggregation_function_instance_name}')
        ...
        # All branches must participate in synchronization to mark themselves ready
        ...
        # The branch that completes last (all_ready=True) MUST invoke the aggregation function
        # to ensure the workflow continues. The conditional is only used for logging/debugging
        # to show which branch was expected to be last.
        should_invoke = self.check_conditional(user_function_output, input_payload, unum_index_list)
        ...
        if all_ready:
            ...
            # fan-in not ready or conditional is false. just return
            if self.debug:
                print(f'[DEBUG] {self.my_node_name}-{unum_index_list} fan-in: all_ready={all_ready}, should_invoke={should_invoke}')
