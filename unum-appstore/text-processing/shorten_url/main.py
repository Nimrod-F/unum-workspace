...
def ingress(event):
    ...
    if event["Data"]["Source"] =="http":
        ...
        if eager_fanin.get("Enabled", False):
            # Eager fan-in with TRANSPARENT LazyInputList
            # Behaves exactly like a regular list - user code doesn't change
            
            poll_interval = float(os.environ.get('UNUM_EAGER_POLL_INTERVAL', '0.1'))
            timeout = float(os.environ.get('UNUM_EAGER_TIMEOUT', '300'))
            
            if unum.debug:
                ready_count = len(eager_fanin.get("Ready", []))
                total = eager_fanin.get("TotalBranches", len(event["Data"]["Value"]))
                print(f'[DEBUG] Eager fan-in: creating LazyInputList. {ready_count}/{total} initially ready')
            
            # Create lazy inputs that fetch data on-demand
            ...
            # Try to pre-resolve inputs that are already ready (non-blocking)
            resolved, pending = lazy_inputs.try_resolve_all()
            
            if unum.debug:
                print(f'[DEBUG] Pre-resolved {resolved} inputs, {pending} still pending')
            
            # For GC, we'll need to collect GC tasks after all inputs are resolved
            # This is handled by the LazyInputList.get_gc_tasks() method
            # We store a reference for later use in egress
            unum._lazy_inputs = lazy_inputs
            unum.fan_in_gc = True  # Will need GC after all resolved
