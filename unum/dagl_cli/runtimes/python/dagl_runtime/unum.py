from dagl_runtime.ds import UnumIntermediaryDataStore
import json
import uuid
import os, sys
...
from dagl_runtime.faas_invoke_backend import InvocationBackend, _load_function_map, _is_multi_platform_map
...
class Unum(object):
    ...
    def __init__(self, config, datastore_type, datastore_name, platform, gc):
        ...
        if 'Next' in config:
            ...
            # Auto-detect multi-platform: if function map has mixed platforms, use 'multi' backend
            effective_platform = platform
            if platform in ('aws', 'multi'):
                fmap = _load_function_map()
                if _is_multi_platform_map(fmap):
                    effective_platform = 'multi'

            self.faas_backend = InvocationBackend.create(effective_platform)
