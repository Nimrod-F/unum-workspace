...
_platform = os.environ.get('FAAS_PLATFORM', 'aws')
_native_platform = os.environ.get('FAAS_PLATFORM_NATIVE', _platform)

_boto3_available = False
if _platform in ('aws', 'multi') and _native_platform != 'gcp':
    try:
        import boto3
        ...
        pass
if _platform == 'gcloud' or _native_platform == 'gcp':
    try:
...
# Redis import (optional — only needed when datastore_type=redis)
...
@UnumIntermediaryDataStore.add_datastore('redis')
class RedisDriver(UnumIntermediaryDataStore):
    """Redis-backed checkpoint/fan-in datastore.
    
    Key schema:
      Checkpoint:  dagl:ckpt:{session}/{instance_name}-output  → JSON string
      GC sync:     dagl:sync:{session}/{parent}-gc             → bitmap (string of 0/1)
      Fan-in sync: dagl:sync:{session}/{agg_func}-fanin        → bitmap (string of 0/1)
      Eager claim: dagl:claim:{session}/{agg_func}-eager-claim → "1"
      Counter:     dagl:ctr:{session}/{counter_name}           → integer
    
    Designed for cross-cloud workflows where DynamoDB is not accessible from GCP.
    Uses the `redis` Python package (same one already used for the function registry).
    
    Constructor accepts Redis URL via ds_name parameter:
      e.g. "redis://hostname:6379/0" or "redis://:password@host:6379/0"
    """

    def __init__(self, ds_name, debug=False):
        super(RedisDriver, self).__init__("redis", ds_name, debug)
        if not _redis_available:
            raise ImportError(
                "Redis driver requires the 'redis' package. "
                "Install with: pip install redis"
            )
        self.client = _redis_module.Redis.from_url(ds_name, decode_responses=True)
    ...
    def log_metrics(self):
        print(f"[METRICS] redis_reads={self._metrics_reads} "
              f"redis_writes={self._metrics_writes} "
              f"redis_deletes={self._metrics_deletes}")

    # ── Key helpers ──────────────────────────────────────────────────────────

    def checkpoint_name(self, session, instance_name):
        return f'dagl:ckpt:{session}/{instance_name}-output'

    def gc_sync_point_name(self, session, parent_function_instance_name):
        return f'dagl:sync:{session}/{parent_function_instance_name}-gc'

    def fanin_sync_point_name(self, session, aggregation_function_instance_name):
        return f'dagl:sync:{session}/{aggregation_function_instance_name}-fanin'

    # ── Checkpoint operations ────────────────────────────────────────────────

    def checkpoint(self, session, instance_name, data):
        """Write checkpoint (idempotent — NX ensures write-once).
        
        @return 1 if written successfully, -1 if already exists.
        """
        key = self.checkpoint_name(session, instance_name)
        value = json.dumps(data)
        
        # SET with NX — only write if key does not exist (idempotent)
        result = self.client.set(key, value, nx=True, ex=3600)  # 1h TTL
        self._metrics_writes += 1
    ...
    def get_checkpoint(self, session, instance_name):
        """Get checkpoint user data, or None if not exists."""
        key = self.checkpoint_name(session, instance_name)
    ...
    def get_checkpoint_full(self, session, instance_name):
        """Get the full checkpoint item (User + GC), or None."""
        key = self.checkpoint_name(session, instance_name)
        raw = self.client.get(key)
        self._metrics_reads += 1
        ...
        if isinstance(data, dict):
            result = {}
            if 'User' in data:
                result['User'] = json.loads(data['User']) if isinstance(data['User'], str) else data['User']
            elif 'Value' in data:
                result['User'] = json.loads(data['Value']) if isinstance(data['Value'], str) else data['Value']
            if 'GC' in data:
                result['GC'] = data['GC']
            return result
        return {'User': data}
    ...
    def read_input(self, session, values):
        """Read multiple checkpoints for fan-in aggregation.
        
        Returns ordered list matching `values` parameter order.
        Each element: {'User': <data>, 'Name': <key>, 'GC': ...}
        """
        keys = [self.checkpoint_name(session, v) for v in values]
        
        # MGET — single round-trip for all keys
        raw_values = self.client.mget(keys)
        self._metrics_reads += len(keys)
        
        results = []
        for i, raw in enumerate(raw_values):
            if raw is None:
                print(f'[WARN] Fan-in checkpoint missing: {keys[i]}')
                continue
            ...
            if isinstance(data, dict):
                if 'User' in data:
                    item['User'] = json.loads(data['User']) if isinstance(data['User'], str) else data['User']
                elif 'Value' in data:
                    item['User'] = json.loads(data['Value']) if isinstance(data['Value'], str) else data['Value']
                if 'GC' in data:
                    item['GC'] = data['GC']
            else:
                item['User'] = data
        ...
        if len(results) < len(values):
            print(f'[WARN] Not all values for fan-in were read from redis')
            print(f'[WARN] Expect {len(values)}. Got {len(results)}')
    ...
    def try_claim_eager_fanin(self, session, aggregation_function_instance_name):
        ...
        @return True if this caller won the claim, False otherwise.
        """
        key = f'dagl:claim:{session}/{aggregation_function_instance_name}-eager-claim'
        result = self.client.set(key, "1", nx=True, ex=3600)
        self._metrics_writes += 1
        return bool(result)
    ...
    def check_checkpoints_exist(self, session, instance_names):
        ...
        @return (ready_list, missing_list) of instance names.
        """
        keys = [self.checkpoint_name(session, v) for v in instance_names]
        ...
        for i, exists in enumerate(results):
            if exists:
                ready.append(instance_names[i])
            else:
                missing.append(instance_names[i])
        
        return ready, missing

    def await_checkpoints(self, session, instance_names, poll_interval=0.1, timeout=300):
        """Poll until all required checkpoints exist."""
        start_time = time.time()
        ...
        while True:
            ...
            if elapsed >= timeout:
                raise TimeoutError(
                    f'Timeout waiting for checkpoints after {elapsed:.2f}s. Missing: {missing}')
            
            if self.debug:
                print(f'[DEBUG] Waiting for {len(missing)} checkpoints: {missing[:3]}...')
            
            time.sleep(poll_interval)

    def read_input_with_await(self, session, values, poll_interval=0.1, timeout=300):
        """Read fan-in inputs, waiting for any that aren't ready."""
        self.await_checkpoints(session, values, poll_interval, timeout)
        return self.read_input(session, values)

    def create_lazy_inputs(self, session, instance_names, poll_interval=0.1, timeout=300):
        """Create LazyInputList for eager fan-in."""
        lazy_inputs = [
            LazyInput(
                datastore=self,
                session=session,
    ...
    def create_future_inputs(self, session, instance_names, ready_names=None, poll_interval=0.1, timeout=300):
        """Create AsyncFutureInputList for future-based execution."""
        ready_set = set(ready_names) if ready_names else set()
        ...
        for name in instance_names:
            ...
            if is_ready:
                checkpoint = self.get_checkpoint_full(session, name)
                futures.append(UnumFuture(
                    datastore=self, session=session, instance_name=name,
                    value=checkpoint.get('User') if checkpoint else None,
                    is_ready=True, poll_interval=poll_interval,
                    timeout=timeout, debug=self.debug
                ))
            else:
                futures.append(UnumFuture(
                    datastore=self, session=session, instance_name=name,
                    is_ready=False, poll_interval=poll_interval,
                    timeout=timeout, debug=self.debug
                ))
        
        return AsyncFutureInputList(futures, debug=self.debug)

    # ── Synchronization (bitmap pattern via Redis strings) ───────────────────

    def gc_sync_ready(self, session, parent_function_instance_name, index, num_branches):
        """Mark GC as ready for this branch, check if all branches done."""
        sync_key = self.gc_sync_point_name(session, parent_function_instance_name)
        return self._sync_ready_bitmap(sync_key, index, num_branches)

    def fanin_sync_ready(self, session, aggregation_function_instance_name, index, my_instance_name, num_branches):
        """Mark fan-in branch as ready, check if aggregation can proceed."""
        sync_key = self.fanin_sync_point_name(session, aggregation_function_instance_name)
        return self._sync_ready_bitmap(sync_key, index, num_branches)

    def _sync_ready_bitmap(self, sync_key, index, num_branches):
        """Atomic bitmap synchronization using Redis SETBIT + BITCOUNT.
        
        Each branch sets its bit. When all bits are set (bitcount == num_branches),
        the last writer returns True (it should trigger the action).
        
        Uses a Lua script for atomicity (set bit + count in one operation).
        """
        # Lua script: set the bit, then count all set bits, return count
        lua_script = """
        ...
        """
        self._metrics_writes += 1
        count = self.client.eval(lua_script, 1, sync_key, str(index))
        return count >= num_branches

    def check_fan_in_complete(self, session, values, target_count):
        """Increment counter and check if fan-in is complete."""
        counter_name = ""
        for v in values:
            counter_name = f'{counter_name}{v}-'
        counter_name = f'{counter_name}counter'
        
        key = f'dagl:ctr:{session}/{counter_name}'
        # INCR is atomic
